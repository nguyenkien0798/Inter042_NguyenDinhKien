...
INSERT INTO `furama`.`khachhang` (`IDKhachHang`, `IDLoaiKhachHang`, `HoTen`, `NgaySinh`, `SoCMND`, `SDT`, `Email`, `DiaChi`) VALUES ('6', '3', 'Nguyen Van E', '1960-06-15', '200012378', '0771593789', 'nguyenvane2@gmail.com', 'Hue');
...
from KhachHang kh 	join HopDong hd on kh.IDKhachHang = hd.IDKhachHang
					join LoaiKhachHang lk on kh.IDLoaiKhachHang = lk.IDLoaiKhachHang
Where lk.TenLoaiKhachHang like 'Diamond'
group by kh.IDKhachHang
order by count(kh.IDKhachHang);

/*Cau 5: Hiển thị IDKhachHang, HoTen, TenLoaiKhach, IDHopDong, TenDichVu, NgayLamHopDong, NgayKetThuc,
...
select kh.IDKhachHang, kh.HoTen, lk.TenLoaiKhachHang, hd.IdHopDong, dv.TenDichVu, NgayLamHopDong, NgayKetThuc, TongTien
from khachhang kh left join hopdong hd on kh.IDKhachHang = hd.IDKhachHang  
				  left join loaikhachhang lk on kh.IDLoaiKhachHang = lk.IDLoaiKhachHang
...
/*Cau 6: Hiển thị IDDichVu, TenDichVu, DienTich, ChiPhiThue, TenLoaiDichVu của tất cả các 
...
/*Cau 9: Thực hiện thống kê doanh thu theo tháng, nghĩa là tương ứng với mỗi 
...
/*Cau 11: Hiển thị thông tin các Dịch vụ đi kèm đã được sử dụng bởi những Khách hàng có TenLoaiKhachHang là “Diamond” và có địa chỉ là 
“Vinh” hoặc “Quảng Ngãi”.*/

select dvdk.IDDichVuDiKem, TenDichVuDiKem, Gia, DonVi, TrangThaiKhaDung, kh.IDKhachHang, DiaChi, TenLoaiKhachHang 
from dichvudikem dvdk join hopdongchitiet hdct on dvdk.IDDichVuDiKem = hdct.IDDichVuDiKem
					  ...
					  join khachhang kh on kh.IDKhachHang = hd.IDKhachHang
					  join loaikhachhang lk on lk.IDLoaiKhachHang = kh.IDLoaiKhachhang
where (kh.DiaChi = "Vinh" or kh.DiaChi = "QuangNgai") and lk.TenLoaiKhachHang = "Diamond";
